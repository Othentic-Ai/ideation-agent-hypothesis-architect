"""CLI entry point for the Hypothesis Architect agent."""
...
@click.group()
def cli():
    """Ideation Agent: Hypothesis Architect - Lean Startup framework."""
    pass
...
def run(session_id: str, problem: str | None, output_json: bool):
    ...
    console.print(f"[blue]Running hypothesis-architect agent for session: {session_id}[/blue]")
    result = run_agent(session_id, problem)
    memory.update_phase("hypothesis_architect", "complete", result)

    if output_json:
        print(json.dumps(result, indent=2))
    else:
        console.print("[green]Hypothesis Architect agent completed successfully[/green]")
